...
- Mesh mode: `solver.meshMode` selects the mesh topology for a `pipe` edge. `planar-2d` (default) is the 2D one-cell-thick planar strip. `axisymmetric` builds a true 3D circular-pipe **wedge** (a ~5-degree wedge revolved about the pipe axis with OpenFOAM `wedge` front/back patches and a collapsed `axis`), generated as a `blockMeshDict` that Allrun's `blockMesh` builds at run time (the fitted planar polyMesh and the `snappyHexMesh` stage are skipped). It is honored only for `incompressible-navier-stokes` on a single straight circular pipe; other modes/geometries fall back to the planar strip. Combined with `runMode: "steady"` it reproduces the 3D Hagen-Poiseuille law `Delta p = 128*mu*L*Q/(pi*D^4)` to sub-percent. See [docs/openfoam-axisymmetric-wedge-2026-07-22.md](docs/openfoam-axisymmetric-wedge-2026-07-22.md).
