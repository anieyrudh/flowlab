...
- `full-ogrid` is a separate experimental mode for exactly one steady, incompressible, laminar, constant-diameter circular source-to-sink pipe. It generates a full-360 five-block all-hex volume with a center block, four conformal circumferential wall blocks, internal interfaces, and only `inlet`, `outlet`, and `walls` boundary patches. Axial, annular-radial, circumferential, and core-side counts are explicit; unsupported topology or physics fails closed. Preview VTK/VTU and solved fields use the genuine XYZ volume, and the cinema renderer/prober operate on its exterior faces and owning hexahedra. The first frozen campaign remains a coarse solver-gate failure. Its prospective v2 successor completed coarse, medium, and fine through the product path and passed every frozen numerical-verification candidate gate, with observed order `1.6126` and fine-grid GCI `0.1597%`. Controlled independent review of the exact immutable package digest is still pending, so no benchmark, independent-validation, promotion, or release claim exists. See [docs/non-axisymmetric-o-grid-status-2026-07-24.md](docs/non-axisymmetric-o-grid-status-2026-07-24.md) and the [v2 controlled-review handoff](docs/validation/full-ogrid-straight-pipe/REVIEW_HANDOFF_V2_2026-07-24.md).
