    ...
    componentResolution: {
      ...
      map: linked
        ? {
            ...
            artifactBindings: [{ artifactName: "*", edgeId: "inlet", scope: "all-cells" }]
          }
        : null
    },
    limits: {
...
test.describe("FlowLab editor workspace", () => {
  ...
  test("builds a bounded imported derived volume as probe-only", async ({ page }) => {
    const consoleErrors: string[] = [];
    page.on("console", (message) => {
      if (message.type() === "error") consoleErrors.push(message.text());
    });
    await installDerivedVisualizationRoutes(page, false);
    await openFresh(page);
    await showStage(page, "Inspect");
    ...
    await expect(page.getByLabel("Derived visualization status")).toContainText("probe-only");
    await expect(page.getByLabel("Derived cut plane axis")).toBeVisible();
    await expect
      .poll(() => page.getByTestId("cinema-canvas").evaluate((canvas) => (canvas as HTMLCanvasElement).dataset.derivedFallback))
      .toMatch(/none|webgl2-required/);
