"""Independent confirmation of the six v2-sensitive cells under v3."""
...
from .laminar_all_hex_campaign import ResourceBudget, WeightedScheduler, _canonical_sha256, _write_json
...
SCHEMA = "flowlab.laminar-all-hex-v3-confirmation.v1"


def is_v2_sensitive_cell(cell: dict) -> bool:
    p = cell.get("parameters", {})
    return (
        cell.get("lane") == "physical-envelope"
        and p.get("reynoldsNumberHeightBased") == 66.7
        and p.get("lengthToHeightRatio") == 1.0
        and (
            (p.get("axialCellAspectRatio") == 1.0 and p.get("level") == "coarse")
            or (
                p.get("axialCellAspectRatio") == 2.0
                and p.get("level") in {"coarse", "medium"}
            )
        )
    )
...
def run_confirmation(campaign: Path, output: Path, capacity: int) -> dict:
    ...
    if not archived_sources_match(campaign):
        raise ValueError("workspace source hashes no longer match the v3 campaign")
    manifest = json.loads(
        (campaign / "campaign-manifest.json").read_text(encoding="utf-8")
    )
    if manifest.get("campaignId") != CAMPAIGN_ID:
        raise ValueError("confirmation source is not laminar-all-hex-v3")
    selected = [cell for cell in manifest["cells"] if is_v2_sensitive_cell(cell)]
    if len(selected) != 6:
        raise ValueError("v3 confirmation must select exactly six v2-sensitive cells")
    originals = {
    ...
    _write_json(
        output / "confirmation-manifest.json",
        {
            ...
            "method": "fresh independent cases with the identical frozen v3 contract",
            "cells": cells,
        },
    )
    ...
    for result in results:
        repeated = json.loads((output / result["result"]).read_text(encoding="utf-8"))
    ...
    checks = {
        ...
        "allSourceCellsAccepted": all(row["status"] == "accepted" for row in originals.values()),
