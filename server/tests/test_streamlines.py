...
def test_su2_planar_streamlines_remain_2d_and_unlinked(tmp_path: Path) -> None:
    ...
    result = derive_streamlines_from_artifact(tmp_path, case, request)

    assert result["spatialDimension"] == 2
    assert result["sourceIdentity"] == "artifact-local-unlinked"
    assert "explicitly 2D and unlinked" in result["linkageNote"]
